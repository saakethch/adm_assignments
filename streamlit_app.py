...
""".format(
        ...
        l.write("Query 1 Connected")
        l.write("Question: List all the states with at least 10 customers who during a given month bought items with the price tag at least 20% higher than the average price of items in the same category.",)
...
def part2():
    ...
    for i, feature in enumerate(features):
        if data_types[i] == int:
            ...
            if feature == "CA_ZIP":
                inputs.append(
                    inp.text_input(f"Enter {feature}", max_chars=5, value="02130")
                )
    ...
    if st.button("Predict CLV"):
        ...
        def predictCLV(_df):
            df_predicted_clv = df.select(*df, predict(*df).alias("PREDICTION"))
...
def app():
    with st.container():
        ...
        part1()
    with st.container():
        st.header("Customer Lifetime Valuation using XGBoost")
        part2()
